...
This project is for owners of the new generation SMA devices (Photovoltaic inverters and Home Managers), who want to control or read out values on their Node-Red based home automation system. The Node-Red server runs typically on a server which is placed in the local network, to which the SMA devices need also be connected. Most Node-Red nodes for SMA devices found in Internet are outdated and support only older devices, but not the newest generation of SMA devices. Following SMA devices (and maybe more?) are currently supported:

* SMA Energy Meter (EMETER-20)
* SMA Sunny Home Manager 2.0 (HM-2.0)
* SMA Sunny Tripower 5.0 / 6.0 / 8.0 / 10.0 Smart Energy (STPx0-3SE)

My goal was to use only basic nodes, like udp- and tcp-nodes (which are part of the basic Node-Red installation) to have the largest flexibility for adjusting/changing later all kind of parameters.

## 1. Overview of the SMA Energy Meter/Home Manager sub-flow

This sub-flow I didn't develop by myself. It is based on the SMA Node-RED flow created originally by [Becker884](https://github.com/Becker884). But I had to make some minor adjustments for my purposes.

![SMA Home Manager flow](/SMA_SunnyHomeManager2.0.png)

The SMA Home Manager sends per default every 1 second automatically a UDP multicast message to target address 239.12.255.254 at port 9522. Whoever client registers for listening to this multicast address can receive this message. The messages with a payload of exactly 608 bytes we are interested in. The UDP node needs to be configured accordingly as shown in the following picture.

![UDP multicast](/udp.png)

## 2. Overview of the SMA PV-inverter sub-flow

This sub-flow I completely developed by myself.
...
## 3. Overall solution flow

Because each sub-flow for its own doesn't provide all values of interest (e.g. the Energy Meter doesn't provide PV/grid/battery-related values) I combined both sub-flows into one flow.
This overall solution flow can be download here: [SMA Node-Red flow](/flow_sma.json).
